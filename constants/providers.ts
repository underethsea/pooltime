...
function createStaticProvider(url: string, chainId: number, name: string): providers.JsonRpcProvider {
  ...
  try {
    provider = new providers.StaticJsonRpcProvider(url, { chainId, name });
  } catch (e) {
    provider = new providers.JsonRpcProvider(url, { chainId, name });
  }
  ...
  if ((provider as any).perform) {
    const originalPerform = (provider as any).perform.bind(provider);
    (provider as any).perform = function(method: string, params: any): Promise<any> {
      if (method === 'getNetwork' || method === 'chainId') {
        return Promise.resolve({ chainId, name });
      }
      return originalPerform(method, params);
    };
  }
