...
async function fetchPromotions(): Promise<any> {
    ...
    return result;}catch(e){console.log(e);return []}
}

function calculateTokensPerSecond(tokensPerEpoch: string, epochDuration: string): number {
...
export async function GetActivePromotionsForVaults(vaults: any[], active = false,priceData:any): Promise<any[]> {
    ...
    for (const chain in data) {
        if (data.hasOwnProperty(chain) && data[chain].length > 0) {
            ...
            const activePromotionsWithTokens = promotions.filter((promo: any) => {
                ...
                const promoEnd = promoStart + parseInt(promo.epochDuration) * parseInt(promo.initialNumberOfEpochs);
                return lowercaseVaultAddresses.includes(promo.vault.toLowerCase()) &&
                    currentTimestamp >= promoStart &&
                    (!active || currentTimestamp < promoEnd); // Add the condition for live here
            });

            // Filter promotions with valid prices from the new price data
            const validPromotionsWithPrices = activePromotionsWithTokens.map((promo: any) => {
                const price = priceData.assets[chain]?.[promo.token.toLowerCase()];
                if (price) {
                    return {
                        ...promo,
                        tokenDecimals: promo.tokenDecimals,
                        tokensPerSecond: calculateTokensPerSecond(promo.tokensPerEpoch, promo.epochDuration),
                        price: price,
    ...
    const promotionsByVault = vaults.reduce((acc, vaultAddress) => {
        const vaultPromotions = allPromotions.filter((promo: any) => promo.vault.toLowerCase() === vaultAddress.toLowerCase());
...
function determineEpochStatus(currentTimestamp:any, promo:any) {
    ...
    const promoEnd = promoStart + epochDuration * parseInt(promo.initialNumberOfEpochs);
